...
- [ ] 2. Create `src/lib/ui.ts` exporting a `buttonClass` map (`primary`/`outline`/`ghost`) and a shared `fieldClass`. Hovers use real color shifts (not opacity); every variant includes a visible `focus-visible:ring-2 focus-visible:ring-secondary-container focus-visible:outline-none`. (No JSX here — class strings only.)
- [ ] 3. Replace the outline-button className in `FollowButton.tsx`, `PokeButton.tsx`, `PokeBackButton.tsx` with `buttonClass.outline` (keep each component's own state/transition logic).
- [ ] 4. Replace the primary-fill button className in `PostForm.tsx`, `WallComposer.tsx`, `CommentSection.tsx`, the feed Quick Search button (`feed/page.tsx`), and login/register with `buttonClass.primary`.
